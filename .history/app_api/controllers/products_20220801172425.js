...
module.exports.getList = function (req, res) {
  ...
  let searchObj = {typeProd : req.params.prodName };

  ProductModel.find( searchObj ,  function (err, product) {
    if (product.length === 0 )
      return sendJSONResponse(res, 400, {
        success: false,
        err: { msg: "Не вірні данні запиту!" },
      });
    if (err)
...
module.exports.getUserSearchList = function (req, res) {
  
  let searchObj =  (req.params.userSearchObj).toString();

  
  ProductModel.find( {title: {$regex:searchObj , $options: "i"}}, function (err, product) {
    if (err)
      return sendJSONResponse(res, 500, {
        success: false,
        err: { msg: "Fetch faild!" },
